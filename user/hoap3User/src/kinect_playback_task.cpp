...
KinectPlayback::KinectPlayback() : start_time_(0), previous_time_(0)
{
	if (Kinect_pipe != 0)
	{
		close(Kinect_pipe);
...
int KinectPlayback::changeParameters()
{
	int ivar;
	double dvar;
...
int KinectPlayback::KinectReader(const char *fifo_name, char *buf)
{
	int i, j, n, n_tmp;
...
void KinectPlayback::kinect2joint(char buffer[])
{
	int i, j, k;
	char *buffer_pt;
...
void KinectPlayback::control_dofs(void)
{
